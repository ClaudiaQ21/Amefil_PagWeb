...
def insertar_tipo_producto(nombre):
    ...
    with conexion.cursor() as cursor:
        cursor.execute("INSERT INTO tipo_producto (nombre) VALUES (%s)",
                       (nombre))
    conexion.commit()
    conexion.close()
...
def obtener_tipo_por_id(id_tipo):
    ...
    with conexion.cursor() as cursor:
        cursor.execute(
            "SELECT * from tipo_producto WHERE id_tipo = %s", (id_tipo))
...
def actualizar_tipo_producto(nombre, id_tipo):
    conexion = obtener_conexion()
    with conexion.cursor() as cursor:
        cursor.execute("UPDATE tipo_producto SET nombre = %s where id_tipo = %s ",
                       ( nombre, id_tipo))
    conexion.commit()
    conexion.close()
...
def obtener_tipo_productos():
    ...
    tipos = None
    with conexion.cursor() as cursor:
        cursor.execute("SELECT id_tipo, nombre, estado from tipo_producto")
...
def actualizar_tipo_producto(nombre, estado, id_tipo):
    conexion = obtener_conexion()
    with conexion.cursor() as cursor:
        cursor.execute("UPDATE tipo_producto SET nombre = %s, estado = %s WHERE id_tipo = %s", (nombre, estado, id_tipo))
    conexion.commit()
    conexion.close()
